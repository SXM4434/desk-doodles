...
// F3-B · Trophy Wall · Path 2 (SVG / vector) — CIS-driven rebuild 2026-05-31.
//
// Replaces the previous F3B_PINS-driven flat-array iteration with CIS subject
// iteration. Each of 14 subjects has its own form choice (memorabilia register).
// fidget + seltzer default OFF (no natural memorabilia form).
//
// Path 1 (3D Trophy Wall) still uses F3B_PINS for now — Trophy Wall Path 1
// migration to CIS is a separate pass.
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        console.log(`[F3-B TrophyWall Path2] ${subject.id}/${form.shape} → ${describeF3SubjectAction(form.action)}`);
      }}
      style={{
        position: 'absolute',
        top: subject.top,
        ...
        animation: staggerMs === 0
          ? undefined
          : `f3b-scatter-reveal 0.7s cubic-bezier(0.22, 1, 0.36, 1) ${index * staggerMs}ms forwards`,
        zIndex: hovered ? 10 : 1,
      }}
    >
      <div
        style={{
          ...
          filter: showGlow ? 'drop-shadow(0 8px 16px rgba(0,0,0,0.14)) brightness(1.05)' : 'drop-shadow(0 1px 2px rgba(0,0,0,0.05))',
        }}
      >
        <SvgStyleTransform>
...
export function F3_B_TrophyWall_Path2() {
