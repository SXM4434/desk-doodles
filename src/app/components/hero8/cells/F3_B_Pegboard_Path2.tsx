...
import { useF3TiltRange, scaleTilt } from '../../../state/F3TiltRangeContext';
...
import { useF3PegboardHang, F3_HANG_CLEARANCE, type F3PegboardHang } from '../../../state/F3PegboardHangContext';
import { SvgStyleTransform } from './SvgStyleTransform';
import {
  F3_PEGBOARD_SUBJECTS,
  F3_PEGBOARD_HEIGHT,
  F3_PEGBOARD_COLS,
  F3_PEGBOARD_PEG_SPACING,
  F3_PEGBOARD_PEG_RADIUS,
  describeF3SubjectAction,
  findPegboardForm,
  type F3PegboardShapeId,
  type F3PegboardForm,
  type F3PegboardSubjectDef,
} from './f3CoreIdentitySet';

// F3-B · Pegboard · Path 2 (SVG / vector) — CIS-driven rebuild 2026-05-31.
//
// Iterates the Core Identity Set (14 subjects). Each subject's form choice
// comes from F3SubjectFormsContext (per-subject 'off' | shape). The PegToolShape
// switch covers every shape in F3PegboardShapeId.

const STROKE = 'var(--dir-text-primary)';
const WASH = 'color-mix(in oklab, var(--dir-text-primary) 8%, transparent)';
// Knockout ink for labels rendered ON darkly-filled regions (e.g. white text
// on a black band). Pulls from the surface bg so it inverts with direction.
const BG_INK_FOR_PEGBOARD = 'var(--dir-bg)';

function PegToolShape({ shape }: { shape: F3PegboardShapeId }) {
  switch (shape) {
    // ── Elara ──────────────────────────────────────────────────────────────
    case 'sketchbook':
      return (
...
// Hanging mechanism render — sits in the clearance zone ABOVE the item top.
// Drawn at 60px wide (centered on the item) regardless of item width; the
// strings reach ~18px each side of center which lands on the top edge of
// most items in the slate. The clearance height comes from F3_HANG_CLEARANCE.
function HangMechanism({
  mechanism,
  clearance,
  showGlow,
}: {
  mechanism: F3PegboardHang;
  clearance: number;
  showGlow: boolean;
}) {
  const STROKE_INK = 'var(--dir-text-primary)';
  // Attach width — how far apart the string endpoints are at the item edge.
  const attachHalf = 16;
  const svgWidth = 60;
  switch (mechanism) {
    case 'peg':
      return (
        <div
          ...
          style={{
            ...
            height: 6,
            borderRadius: '50%',
            backgroundColor: STROKE_INK,
            boxShadow: showGlow
              ? '0 0 0 3px color-mix(in oklab, var(--dir-text-primary) 18%, transparent)'
              : 'none',
    ...
    case 'string-triangle':
      // Anchor at top-center of clearance zone; two strings descend to item
      // top-left and top-right (~attachHalf from center). Forms triangle.
      return (
        <svg
          aria-hidden
          width={svgWidth}
          height={clearance + 2}
          viewBox={`0 0 ${svgWidth} ${clearance + 2}`}
          style={{
            position: 'absolute',
            bottom: -1,
            left: '50%',
        ...
        >
          {/* Slight curve in each leg via quadratic — natural string sag */}
          <path
            d={`M ${svgWidth / 2} 2 Q ${svgWidth / 2 - attachHalf / 2} ${clearance * 0.6} ${svgWidth / 2 - attachHalf} ${clearance}`}
            fill="none"
            stroke={STROKE_INK}
            strokeWidth="0.9"
            strokeLinecap="round"
          ...
          <circle
            ...
            style={
              showGlow
                ? { filter: 'drop-shadow(0 0 3px color-mix(in oklab, var(--dir-text-primary) 35%, transparent))' }
                : undefined
            }
          />
        </svg>
    ...
    case 'string-arc':
      // String attached at item top-left + top-right, arcs UP and over the
      // peg hook at top-center. Inverted-U shape — single string, not two.
      return (
        <svg
          aria-hidden
          width={svgWidth}
        ...
        >
          {/* Quadratic arc from one corner up over the peg to the other corner */}
          <path
            d={`M ${svgWidth / 2 - attachHalf} ${clearance} Q ${svgWidth / 2} ${-clearance * 0.2} ${svgWidth / 2 + attachHalf} ${clearance}`}
            fill="none"
            stroke={STROKE_INK}
            strokeWidth="0.9"
            strokeLinecap="round"
          />
          {/* Peg hook at apex of the arc */}
          <circle
            cx={svgWidth / 2}
            cy={clearance * 0.18}
            r="2"
    ...
    case 'pin':
      // Pin/tack at item top — pin head + short shaft pointing down into the
      // item. No anchor above; item hangs directly off the pin.
      return (
        <svg
          aria-hidden
          width={svgWidth}
        ...
        >
          ...
          />
          {/* Pin shaft tapering into the item */}
          <path
            d={`M ${svgWidth / 2 - 1.4} ${clearance - 4} L ${svgWidth / 2 + 1.4} ${clearance - 4} L ${svgWidth / 2} ${clearance + 2} Z`}
            fill={STROKE_INK}
          />
        </svg>
      );
...
function PeggedTool({
  ...
  // Vertical clearance for the hang mechanism. The item top sits clearance
  // px below the subject's `top` anchor so the mechanism has room to render.
  const clearance = F3_HANG_CLEARANCE[hangMechanism];
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        console.log(`[F3-B Pegboard Path2] ${subject.id}/${form.shape} → ${describeF3SubjectAction(form.action)}`);
      }}
      style={{
        position: 'absolute',
        top: subject.top,
        ...
        animation: staggerMs === 0
          ? undefined
          : `f3b-pegboard-reveal 0.6s cubic-bezier(0.22, 1, 0.36, 1) ${index * staggerMs}ms forwards`,
        zIndex: hovered ? 10 : 1,
      }}
    >
      {/* Hang mechanism zone — clearance px tall, mechanism renders inside */}
      <div
        style={{
          position: 'relative',
          height: clearance,
          width: 60,
          marginLeft: -30 + 30, // visual anchor; wrapper already centered via translateX
        }}
      >
        <HangMechanism mechanism={hangMechanism} clearance={clearance} showGlow={showGlow} />
      </div>
      <div
        style={{
          transform: `rotate(${scaledTilt + (showLoosen ? -scaledTilt * 0.5 : 0)}deg) scale(${showScale ? 1.06 : 1})`,
      ...
      >
        ...
        <span style={{ marginLeft: 8, color: 'var(--dir-text-secondary)', fontWeight: 400 }}>
          {form.caption}
        </span>
      </div>
    </div>
...
function PegboardSurface({ width, height }: { width: number; height: number }) {
  return (
    <svg
      width={width}
      height={height}
    ...
    >
      <defs>
        <pattern
          ...
          x="0"
          y="0"
          width={F3_PEGBOARD_PEG_SPACING}
          height={F3_PEGBOARD_PEG_SPACING}
          patternUnits="userSpaceOnUse"
        >
          <circle
            cx={F3_PEGBOARD_PEG_SPACING / 2}
            cy={F3_PEGBOARD_PEG_SPACING / 2}
            r={F3_PEGBOARD_PEG_RADIUS}
            fill="var(--dir-detail)"
          />
        </pattern>
      </defs>
      <rect x="0" y="0" width={width} height={height} fill="url(#pegboard-holes)" />
    </svg>
  );
}

export function F3_B_Pegboard_Path2() {
  ...
  const { pegboard: formChoices } = useF3SubjectForms();
  const { state: hangMechanism } = useF3PegboardHang();
  const staggerMs = STAGGER_MS[staggerMode];

  const pegColPct = (col: number): string => {
    const padding = 10;
    const usable = 100 - padding * 2;
    const step = usable / (F3_PEGBOARD_COLS - 1);
    return `${padding + col * step}%`;
  };

  const visibleSubjects = F3_PEGBOARD_SUBJECTS
    .map((subject) => {
      const choice = formChoices[subject.id];
      if (choice === 'off') return null;
      const form = findPegboardForm(subject.id, choice);
      if (!form) return null;
      return { subject, form };
    })
    .filter((x): x is { subject: F3PegboardSubjectDef; form: F3PegboardForm } => x !== null);
  ...
  return (
    <div
      style={{
        position: 'relative',
        width: '100%',
        height: F3_PEGBOARD_HEIGHT,
        backgroundColor: 'color-mix(in oklab, var(--dir-text-primary) 3%, var(--dir-bg))',
        border: '1px solid var(--dir-border)',
        borderRadius: 4,
        overflow: 'hidden',
      }}
    >
      <style>{`
        @keyframes f3b-pegboard-reveal {
          from { opacity: 0; transform: translateX(-50%) translateY(-6px); }
          to { opacity: 1; transform: translateX(-50%) translateY(0); }
        }
      `}</style>
      <PegboardSurface width={1000} height={F3_PEGBOARD_HEIGHT} />
      {visibleSubjects.map(({ subject, form }, idx) => (
        <div
          key={subject.id}
          style={{
            position: 'absolute',
            top: 0,
            left: pegColPct(subject.pegCol),
            width: 0,
            height: 0,
          }}
        >
