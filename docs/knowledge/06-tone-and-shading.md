...
- **→ [12-the-creation-loop.md](12-the-creation-loop.md)** — the round-7 tone-fill / shade brush is the *input* surface for this page's currency: it's how source darkness (I-2) finally enters a drawing (8 discrete bands stored as record data, never baked). Edge: *the brush supplies the darkness this page spends*.
- **→ [13-the-3d-system.md](13-the-3d-system.md)** — `surface-hatch(band)` carries a region's darkness band into 3D *exactly* (the 3D shader quantizes with the same 8 numbers), which is why hatch (not relief) is the primary 2D→3D shading mapping. Edge: *the same currency, preserved into the third dimension*.
