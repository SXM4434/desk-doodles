...
- **→ [15-the-smart-ml-ladder.md](15-the-smart-ml-ladder.md)** — the dated ladder whose post-makeathon rungs (T2-b LR weights → decision-tree provider → cached-LLM ingest → Snorkel/conformal/RIPPER → role-agnostic schema → manual-toggle layer → meta-engine) ARE this page's trained future; plus the receipts retrofit that feeds the dataset every stream needs. Edge type: *the build sequence toward this layer*.
- **→ [12-the-creation-loop.md](12-the-creation-loop.md)** — naming-as-label is dataset stream 1; the minting moment is where the label is collected. Edge type: *populates the dataset*.
