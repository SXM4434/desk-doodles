...
### D-7 amendment (Sebs follow-up 2026-06-11): Pen mode MUST have a live preview squiggle

The gap Sebs kept probing: with Global OFF, moving a slider changes nothing visible on the desk → the controls FEEL broken even though they're correctly aiming at the next doodle. REQUIRED FIX: a small live PREVIEW STROKE at the top of the desk panel (a sample squiggle rendered through the current pen settings) that re-renders instantly on every control change — the Procreate Brush Studio pattern (brush panel never repaints the canvas; the preview stroke gives the hand immediate feedback). Instant per the motion research (direct manipulation = no tween). This preview IS what makes Pen mode legible; ship it with the Pen|Desk gate, not after.

### D-7 amendment 2: what Desk mode initializes to (Sebs 2026-06-11 "what becomes the default global look")

NO CONVERSION STEP EXISTS. Desk mode is the D-1 lens: flipping Global ON re-renders every object through the CURRENT PEN SETTINGS — the exact values already in the panel (panel never jumps, never lies; one set of values, two scopes). Records untouched; flipping OFF lifts the lens and every object returns to its own render_config. Rejected alternatives: neutral-default seed (panel values would jump on flip), latest-object seed (hidden rule), real conversion (destroys saved looks + others' art). The default global look IS your pen at the moment you flip it.

### D-7 amendment 3 (Sebs 2026-06-11 "some things DO have to convert"): the two globals

TWO different globals; only one converts. (1) STYLE SWEEP (2D Desk mode): never converts — objects are strokes + config; the sweep re-renders strokes under the pen's values (styles were never baked in). Heterogeneous starting styles are irrelevant. (2) MODE FLIP (2D→3D): REAL conversion, via the D-3 ladder, always FROM THE RECORD (strokes), never from the rendered look: strokes → geometry engines (rod/extrude/inflate/solid, content-hash cached); no strokes → extrude-from-outline, else honest paper-card; pinned-3D → as-is (own 2D render in 2D view, D-5). Style differences between objects move to the 3D material/NPR layer (shared Hatch now per D-4; Free Stroke style-system port at M8) — geometry is style-independent. One sentence: the sweep re-renders, the flip converts from strokes.

### D-7 amendment 4 (Sebs 2026-06-11, SUPERSEDES the "auto per object" default in amendment 3): Global means the TOGGLES own the desk — all of them, including mode + geometry

Sebs correction: "global means all objects will be tied to what's currently in the toggles." ONE rule, no special cases: **Global ON → every object renders from the CURRENT panel state — the 2D style toggles when mode=2D, the 3D toggles (geometry dropdown + 3D style + shading sliders) when mode=3D. The mode flip is just another toggle Global sweeps, in BOTH directions (3D→SVG renders everything under the current 2D toggles too, including 3D-native objects via their 2D source per D-5).** The 3D panel section mirrors the 2D grammar: geometry dropdown (Auto / Rod / Extrude / Inflate / Solid) + 3D style dropdown (Hatch interim per D-4; per-style twins after M8). "Auto" = the geometry dropdown's DEFAULT VALUE (each object's shape decides) — a position on the toggle, not a hidden rule; set it to Rod and everything is rods. **Global OFF → the mixed desk: every object is its own record (style AND mode), side by side.** Two desk states: OFF = records rule (mixed), ON = toggles rule (uniform).

### D-7 amendment 5 (Sebs 2026-06-12, RATIFIED): Sketch|Style canvas mode keeps the CLEAN SWAP

The create/redraw canvas mode is an explicit Sketch|Style pill pair (pen-up never transforms; styling is opt-in). RATIFIED: Style mode shows ONLY the styled render (clean swap) — the UX audit's "lens overlay" alternative (raw ink faintly visible under the styled render) was presented and REJECTED by Sebs ("keep what we have"). Do not re-litigate without his ask.
