...
| Hatch | ✓ mark grammar (Hachure/Cross-hatch/Stipple/Contour) + direction mode (Fixed/Light-following) pill rows | ✓ gap/angle/strokeWidth/inkIntensity |
| Native | ✓ 6 material presets | ✓ Polish (highlight tightness) · Reflection (env amount, bounded) · Sheen · Outline (ink edge weight) |

**GAP CELLS LANDED 2026-06-13** (gap-cells rock — fired after be7aac7). Both ✗ cells are now ✓. Hatch grammar/direction = `HatchGrammar`/`HatchDirection` (hatchMaterial.ts shader modes 0/1/2/8 + light-following `u_directionMode`/`u_lightDirView` per-frame; Contour orients marks off the view-normal). All four grammars feed the SAME COVERAGE_BANDS lambert (one math, four renderers — a band-5 region is equally dark in every grammar, proven by the grammar×band contact sheet). Native dials = `NativeProps3D` (materials3d.ts `applyNativeProps`, neutral-identity guaranteed: polish/reflection/sheen 0.5 + outline 0 == the preset unchanged, byte-identical). Reflection drives `reflectivity` + bounded `envMapIntensity` (≤ `REFLECTION_CEIL` 1.8) + clearcoat presence — `envMapIntensity` alone is a dead lever against the de-saturated graphite env (measured Δ0 px), so the visible effect is `reflectivity`; the TAN BOUND holds (battery: reflection MAX on glossy slab oblique = Δr−b < 25, ink-black). Outline = inverted-hull silhouette in ink. State in Canvas3DContext; pills + dials in Canvas3DChrome; threaded via DeskDoodlesCanvas → Stroke3DScene. Battery: `tools/3d/gapcell-battery.{html,-harness.tsx,.mjs}` (grammar×band contact sheet · Fixed-vs-Light orbit · 4-dial min/max strips · tan re-assert · default-identity sha1) — ALL GATES PASS, all boards read; the be7aac7 72-cell material battery still 72/72 (no-props path untouched). Boards: /tmp/dd-gapcell/.
