// Isolate the rim spikes: same disc, svg-port 3D + deep relief, rendered in CLEAN
// vs ROUGH style. Clean has NO wobble — if its rim is smooth, the rough-style
// spikes are silhouette wobble (by design); if clean ALSO spikes, it's a real
// displacement/geometry tear. Tight rim crops + __rimFeatherDbg per style.
...
await orbit(70, 40);
// deep relief on
await p.evaluate(()=>{const w=window;w.__sealedRelief=1;w.__sealedReliefTune={scale:0.45};});
const clip = { x: 500, y: 360, width: 460, height: 340 };
for (const style of ['clean', 'rough']) {
  await set('setSvgStyle', style); await sleep(1500);
  await p.evaluate(()=>{const w=window;if(w.__sealedReliefApply)w.__sealedReliefApply();});
  await sleep(2600);
  const dbg = await p.evaluate(()=>window.__rimFeatherDbg||null);
  await p.screenshot({ path: `/tmp/dd-shots/rim-${style}.png`, clip });
  console.log(style, 'dbg:', JSON.stringify(dbg));
}
