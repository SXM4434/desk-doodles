// Draw the 9-stroke sun, set explicit Extrude, dump conversion receipts so we see
// what each stroke became (treatment / directive / geometry / firedRules).
...
await p.evaluate(() => { if (window.__dd_conversionLog) window.__dd_conversionLog.clear(); });
...
const recs = await p.evaluate(() => {
  const log = window.__dd_conversionLog?.get?.() || [];
  return log.filter((r) => r.entryType === 'conversion').map((r) => ({ id: r.unitId, treat: r.treatment, dir: r.directive, geo: r.geometry, closure: r.closure, rules: (r.firedRules || []).slice(0, 2) }));
});
console.log('extrude receipts:'); for (const r of recs) console.log(' ', JSON.stringify(r));
console.log('count', recs.length, 'errors', errs.slice(0, 3));
await b.close();
