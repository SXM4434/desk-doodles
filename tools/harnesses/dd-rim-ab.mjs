// Clean A/B of the rim feather: SAME disc + deep relief, feather OFF (tiny) vs ON.
// Forces a rebuild between variants (dd-relief nonce). Tight rim crops.
...
for (const [name, frac] of [['OFF', 1e-7], ['ON', 0.06]]) {
  await p.evaluate((f)=>{window.__svgPortRimFeather=f;if(window.__sealedReliefApply)window.__sealedReliefApply();}, frac);
  await sleep(2800);
  const dbg = await p.evaluate(()=>window.__rimFeatherDbg||null);
  await p.screenshot({ path: `/tmp/dd-shots/rimab-${name}.png`, clip });
